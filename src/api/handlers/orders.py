...
async def create_order(
    ...
    result = await service.create_order(
        user=user, new_order=order, session=session
    )
    log_service.info(
...
@router.get("/{user_id]/{order_id}/", status_code=status.HTTP_200_OK)
async def get_user_orders(
    user: TokenData = Depends(get_current_user),
    session: AsyncSession = Depends(get_session),
):
    orders = await service.get_user_orders(user=user, session=session)
    log_service.info(
        "order retrieved successfully: ", orders=user.user_id, email=user.email
    )
    return orders
